...
# Funkcja pomocnicza dla QuickSelect, wybór pivotu z metodą Median of Medians
def median_of_medians(arr, k):
    """ Funkcja zwraca k-ty element z listy arr (posortowaną wersję) przy pomocy Median of Medians """
    # Podziel listę na grupy po 5 elementów
    groups = [arr[i:i + 5] for i in range(0, len(arr), 5)]
    # Oblicz mediany z każdej grupy
    medians = [sorted(group)[len(group) // 2] for group in groups]

    if len(medians) <= 5:
        pivot = sorted(medians)[len(medians) // 2]
    else:
        # Rekursywnie znajdź medianę z median
        pivot = median_of_medians(medians, len(medians) // 2)

    return pivot


# Funkcja do obliczania mediany z wykorzystaniem QuickSelect (solution2) z optymalizacją wyboru pivotu
def quick_select(arr, k):
    if len(arr) == 1:
        return arr[0]

    # Użycie mediany z median jako pivot
    ...
    # Podział na 3 listy: lows, pivots, highs
    for num in arr:
        if num < pivot:
            lows.append(num)
        elif num > pivot:
    ...
    # Wybór odpowiedniej strony w zależności od k
    if k < len(lows):
